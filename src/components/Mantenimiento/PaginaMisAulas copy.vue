          ...
          <template v-if="isDesktop">
            ...
            >
              <template #item.dias="{ item }">
                <div v-for="(dc, index) in item.dias" :key="index" class="py-1">
                  • {{ dc.dia }} - {{ dc.curso }}
                  <v-btn icon small @click="abrirDialogEditDiaCurso(dc, item)">
                    <v-icon>mdi-pencil</v-icon>
                  </v-btn>
                </div>
                <!-- Botón para agregar un nuevo día/curso en este rango -->
                <v-btn
                  small
                  color="success"
                  variant="text"
                  class="mt-2"
                  @click="abrirDialogNuevoDiaCurso(item)"
                >
                  + Agregar Día/Curso
                </v-btn>
              </template>
...
function abrirDialogEditDiaCurso(dc, rango) {
  ...
  Object.assign(datosDiaCurso, {
    ...
    aede_id: buscarCursoIdPorNombre(dc.curso),
    hesc_estado: 'A'
  })
  dialogDiaCurso.value = true
}

// Helpers para obtener IDs (si tu API no devuelve día_id y aede_id directos)
...
function buscarCursoIdPorNombre(nombreCurso) {
  const cursoObj = listaCursos.value.find(c => c.aede_nombre === nombreCurso)
  return cursoObj ? cursoObj.aede_id : null
}
...
async function guardarDiaCurso() {
  ...
  if (!datosDiaCurso.hesc_id) {
    ...
    const data = {
          ...
          ai_aede_id: datosDiaCurso.aede_id,
          av_profile: profile
    }
    axios.post(baseUrl, data, config)
      .then(response => {
...
async function fetchCursos() {
  ...
  const profile = localStorage.getItem("profile") || "demo"
  try {
    const { data } = await axios.get('https://amsoftsolution.com/amss/ws/wsListaAreaEducativaDetalle.php', {
      params: {
        ac_indicador_todos: 'N',
        av_profile: profile
      },
