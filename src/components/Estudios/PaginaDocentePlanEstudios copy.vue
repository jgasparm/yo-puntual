...
async function obtenerPlanesEstudio() {
  const { data } = await axiosInstance.get(`wsConsultaDocentePlanEstudios.php?ai_usua_id=${usua_id}&ac_anio_escolar=${anioEscolar}&av_profile=${profile}`)
  if (data.status) planesEstudio.value = data.data
}

async function obtenerPlanCurricular(id) {
  const { data } = await axiosInstance.get(`wsConsultaPlanCurricular.php?ai_usua_id=${usua_id}&ai_ples_id=${id}&av_profile=${profile}`)
  if (data.status) {
