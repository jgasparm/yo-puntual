...
async function calendarioEscolar() {
  try {
    ...
    if (response.data.status) {
      // Si data no es un arreglo, forzamos un arreglo vacío
      const apiData = Array.isArray(response.data.data)
        ? response.data.data
        : []

      events.value = apiData         // ← siempre Array
      filterByMonth()
    } else {
      // La API respondió con status=false → limpiamos los eventos
      events.value = []
...
function filterByMonth() {
  const safeEvents = Array.isArray(events.value) ? events.value : []
  if (!selectedMonth.value) {
    filteredEvents.value = safeEvents
  } else {
    filteredEvents.value = safeEvents.filter(event => {
      const start = parseLocalDate(event.caes_fecha_inicio)
      return (start.getMonth() + 1) === selectedMonth.value
    })
  }
}
